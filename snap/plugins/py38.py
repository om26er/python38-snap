...
class PythonPlugin(snapcraft.BasePlugin):
    ...
    def build(self):
        ...
        self._run('rm -rf site-packages')
        self._run('mkdir site-packages')

        if self.options.python_packages and '__none__' in self.options.python_packages:
            return

        if self.options.python_packages or self.options.source:
            self._run('/usr/bin/python3.8 -m ensurepip')

        if self.options.python_packages:
            self._run('/usr/bin/python3.8 -m pip install -t site-packages {}'.format(
